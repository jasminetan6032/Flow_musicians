...
addpath('C:\Users\Caro\Documents\MATLAB\connectivity\headinhead');
...
%load eeg_sens; %load some eeg 3D-channels locations; 31 channels 
%clear para;para.rot=180; %setting this rotates locations by 180 degrees
%locs_2D=mk_sensors_plane(eeg_sens); %make a variable locs_2D 
...
clear para;
para.scale=[-0.0002 0.0002];
figure;showcs(imag(cs2_ll),locs_2D,para);  %show the imaginary part of the cross-spectrum

%If you want to show a potential or field you can use 
% the program showfield.
% E.g., to show the eigenvector of largest eigenvalue of the real 
% part of CS, calculated by
[u,s,v]=svd(real(cs2_hl));umax=u(:,1);
% then you can display it as:
figure;showfield(umax,locs_2D);
% 
...
figure;showcs(cs,locs_2D,para); %plots 50 channels as 'head-in-head' with the above changes 
...
clear para;
para.scale=[-2.5 2.5];  % changes the colormap to range from -2 to 2
figure;showcs(tINC4_HL,locs_2D,para); %plots 30 channels as 'head-in-head' with the above changes 

% Finally, if you don't won't to shift circles you can use: 
clear para;para.rot=90;
para.circle_shift=0; %omits shifting
locs_2D=mk_sensors_plane(locs_247,para); %make location file 
