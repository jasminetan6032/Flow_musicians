...
%   $Revision: 5.41.4.18 $  $Date: 2011/05/17 02:32:26 $
%
...
if xOffset && ~isreal(x)
    error(message('MATLAB:interp1:ComplexX'))
end

if ~ppOutput && ~isreal(xi)
    error(message('MATLAB:interp1:ComplexInterpPts'))
end

% Error check for NaN values in X and Y
% check for NaN's
if xOffset && (any(isnan(xCol)))
    error(message('MATLAB:interp1:NaNinX'));
end

% NANS are allowed as a value for F(X), since a function may be undefined
% for a given value.
if any(isnan(yMat(:)))
    warning(message('MATLAB:interp1:NaNinY'));
end
...
if any(h == 0)
    error(message('MATLAB:interp1:RepeatedValuesX'));
end
    ...
    otherwise % 'nearest', 'linear', 'v5cubic'
        ...
        switch method(1)
            case 'n'  % 'nearest'
                ...
                k(i) = k(i)+1;
                yiMat(p,:) = yMat(k,:);
                nanidx = find(isnan(xiCol));
                yiMat(p(nanidx),:) = yiMat(p(nanidx),:)*NaN;
            case 'l'  % 'linear'
                if eqsp
                    s = (xiCol - xCol(k))/h;
                else
...
if ~isequal(extrapval,'extrap')
    if ischar(extrapval)
        error(message('MATLAB:interp1:InvalidExtrap'))
    elseif ~isscalar(extrapval)
        error(message('MATLAB:interp1:NonScalarExtrapValue'))
    end
    if isempty(p)
        p = 1 : numelXi;
    end
...
%-------------------------------------------------------------------------%
    function pp = ppinterp
        ...
        switch method(1)
            ...
            otherwise
                error(message('MATLAB:interp1:ppinterp:UnknownMethod'));
        end
