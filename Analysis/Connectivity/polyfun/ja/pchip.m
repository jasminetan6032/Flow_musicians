...
% 区分的キュービックエルミート内挿の区分的多項式を与えます。
% これは、後で、PPVAL とスプラインユーティリティ UNMKPP で使用できます。
% X は、ベクトルでなければなりません。
% Y がベクトルの場合、 Y(j) は、 X(j) で一致する値がとられます。
% 従って、Y は、X と同じ長さである必要があります。
% Y が行列 または ND 配列の場合、Y(:,...,:,j) は、X(j) で一致する値として
% とられ、そのため、Y の最後の次元は length(X) に等しくなる必要があります。
%   
% YY = PCHIP(X,Y,XX) は、YY = PPVAL(PCHIP(X,Y),XX) と同じです。
% 従って、YY に、XX での補間の値を与えます。
%
